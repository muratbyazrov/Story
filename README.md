...
же [вид](#Пример-запроса), что, по-моему, мнению очень удобно.
...
- Модуль содержит три основных метода:
    - `run` - основной метод
        - Подключается к серверу rabbitMQ, используя настройки из файла конфигурации (host, port, user, password)
        - Вызывает приватный метод `cunsume`
    - `consume` - метод, запускает процесс прослушивания и обработки сообщений
        - Создает канал (`channel`)
        - В рамках канала создает очередь со следующими настройками:
            - `queue` - *string* - Название очереди, которую будем слушать
            - `durable` - *boolean* - если true, то очередь будет сохранять свое состояние и восстанавливается после
            ...
            - `arguments` - необязательные аргументы
                - `x-message-ttl(x-message-time-to-live)` - позволяет установить время истечения срока жизни сообщения в
                  миллисекундах. Если создание очереди происходит с установленным значением аргумента x-message-ttl, то
                  такая очередь будет автоматически исключать сообщения, у которых истек срок действия.
                - `x-max-length` - задает максимальное число сообщений в очереди. Если число сообщений в очереди
                  начинает превышать максимальное число, то начинают удаляться самые старые
