...
- Использует настройки конфига:
  ...
  - `queueDurable` - *boolean* - если true, то очередь будет сохранять свое состояние и восстанавливается после перезапуска сервера/брокера. (По умолчанию `false`)
  ...
  - `noAck`- *boolean* - если `false`, то будет требоваться ручное подтверждение, что сообщение было прочитано и обработано. Иначе сообщение будет висеть в очереди. (По умолчанию `true`)
...
- Использует настройки конфига:
  - `exchange`
  - `queue`
  - `persistent` - *boolean* - если `true`, сообщения не потеряются даже при аварии на сервере rabbitMQ. Это работает в паре с `durable: true`
