...
- Модуль содержит три основных метода:
    ...
    - `consume` - метод, запускает процесс прослушивания и обработки сообщений
        - Подтягивает параметры из конфига:
            ...
            - `queue` - *string* - Название очереди, которую создаем и будем слушать
            - `queueDurable` - *boolean* - если true, то очередь будет сохранять свое состояние и восстанавливается после
            ...
            - `noAck`- *boolean* - если `false`, то будет требоваться ручное подтверждение, что сообщение было прочитано
              и обработано. Иначе сообщение будет висеть в очереди
            - `prefetchCount` - Максимальное количество сообщений, принимаемых потребителем за раз
            - `exclusive` - *boolean* - если true, то очередь будет разрешать подключаться только одному потребителю
            - `bindQueuePattern` - *string* - 
            - `xMessageTtl` - *number* - время жизни сообщений. По умолчанию 10 минут. После этого сообщения удаляются
